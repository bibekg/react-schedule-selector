...
const getTestSchedule = () => [
  moment(startDate).startOf('day').add(12, 'h'),
  moment(startDate).startOf('day').add(1, 'd').add(13, 'h'),
]
...
describe('ScheduleSelector', () => {
  ...
  describe('updateAvailabilityDraft', () => {
    it.each([
      ...
      (type, amount, done) => {
        const start = moment(startDate).add(5, 'hours').toDate()
        const end = moment(start).add(amount, 'hours').toDate()
  ...
  describe('minute-level resolution', () => {
    ...
    it('formats the time column using the timeFormat prop', () => {
      ...
      expect(component.find('ScheduleSelector__TimeText').at(1).render().text()).toEqual('1:15AM')
      // 5-minute resolution
      const componentTwo = shallow(<ScheduleSelector minTime={1} maxTime={2} timeFormat="h:mma" hourlyChunks={12} />)
      expect(componentTwo.find('ScheduleSelector__TimeText').at(1).render().text()).toEqual('1:05AM')
    })
  })
})
